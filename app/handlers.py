...
def register_handlers(dp):
    ...
    dp.message.register(cmd_shutdown, F.text == "Выключить компьютер🔴")
    ...
    dp.message.register(cmd_restart_bot, F.text == "Перезагрузка🔄")
    ...
    dp.message.register(cmd_sleep, F.text == "Обычный сон💤")
    ...
    dp.message.register(cmd_restart_pc, F.text == "Перезагрузить ПК🔄")
    dp.message.register(cmd_restart, F.text == "Перезапустить бота🔄")
    ...
    dp.message.register(cmd_favorites, F.text == "🖥️ Избранное")
    ...
    dp.message.register(handle_delete_program_button, F.text == "🗑️ Удалить программу")
    ...
    dp.message.register(handle_video_buttons, F.text == "⏮️")
    dp.message.register(handle_video_buttons, F.text == "⏸️")
    dp.message.register(handle_video_buttons, F.text == "⏭️")
    dp.message.register(handle_video_buttons, F.text == "🔊+")
    dp.message.register(handle_video_buttons, F.text == "🔉-")
    # Новые кнопки
...
async def handle_block_programs_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    if data == "block_add_exe":
        ...
        await callback.message.edit_text(
            "➕ **Добавить exe для блокировки:**\n\n"
            "Введите имя exe файла (например: chrome.exe)\n\n"
            "При запуске этой программы она будет закрыта и воспроизведется звук."
        )
        await callback.answer()
