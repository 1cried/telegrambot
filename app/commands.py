...
class AutoClickerState(StatesGroup):
    waiting_for_coords = State()
    waiting_for_clicks = State()
...
🔐 Админ:
/set_admin - Установить админа
/password - Сменить пароль
/block - Заблокировать программу
...
async def cmd_set_clipboard(message: types.Message, state: FSMContext):
    ...
    await message.answer("📝 Введите текст для добавления в буфер обмена:")
...
async def show_detailed_gpu(message: types.Message):
    ...
    try:
        ...
        if GPUtil:
            ...
            response += "⚠️ GPUtil не установлен\n"
...
async def handle_notification(message: types.Message, state: FSMContext):
    ...
    subprocess.Popen([
        ...
        f'$textNodes.Item(0).AppendChild($template.CreateTextNode("Уведомление")) | Out-Null; '
        f'$textNodes.Item(1).AppendChild($template.CreateTextNode("{text}")) | Out-Null; '
        f'$toast = [Windows.UI.Notifications.ToastNotification]::new($template); '
        f'[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("Уведомление").Show($toast)'
...
# Хранит позицию для клика
click_position = {'x': None, 'y': None}

# Глобальные переменные для кликера
click_thread = None
stop_event = None

# Windows API для быстрого клика
MOUSEEVENTF_LEFTDOWN = 0x0002
MOUSEEVENTF_LEFTUP = 0x0004

def fast_click():
    """Быстрый клик через WinAPI"""
    ctypes.windll.user32.mouse_event(MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0)
    ctypes.windll.user32.mouse_event(MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)

def click_loop(cps: float, stop_event):
    """Кликер в отдельном потоке"""
    import time
    interval = 1.0 / cps
    while not stop_event.is_set():
        fast_click()
        # Точный сон с проверкой остановки
...
def run_autoclick(cps: float):
    """Запускает кликер в потоке"""
    import threading
    global click_thread, stop_event
    stop_event = threading.Event()
    click_thread = threading.Thread(target=click_loop, args=(cps, stop_event))
    click_thread.start()

def stop_autoclick():
    """Останавливает кликер"""
    global stop_event
    if stop_event is not None:
        stop_event.set()


async def cmd_autoclicker(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("Введи сколько кликов в секунду:")
    await state.set_state(AutoClickerState.waiting_for_clicks)


async def handle_autoclicker_callback(callback: types.CallbackQuery, state: FSMContext):
    """Обработка автокликера"""
    data = callback.data
    
    if data == "autoclick_capture":
        # Запомнить текущую позицию
        x, y = pyautogui.position()
        click_position['x'] = x
        click_position['y'] = y
        await callback.answer(f"Запомнено: X={x}, Y={y}")
        await cmd_autoclicker(callback.message, state)
    
    elif data == "autoclick_click":
        # Клик по запомненной позиции
        if click_position['x'] is not None:
            pyautogui.click(click_position['x'], click_position['y'])
            await callback.answer("👆 Клик!")
            await callback.message.edit_text(f"✅ Клик на X={click_position['x']}, Y={click_position['y']}", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="autoclick_back")]]))
        else:
            await callback.answer("Сначала запомни позицию!")
    
    elif data == "autoclick_double":
        if click_position['x'] is not None:
            pyautogui.doubleClick(click_position['x'], click_position['y'])
            await callback.answer("👆👆 Двойной!")
            await callback.message.edit_text("✅ Двойной клик!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="autoclick_back")]]))
        else:
            await callback.answer("Сначала запомни позицию!")
    
    elif data == "autoclick_right":
        if click_position['x'] is not None:
            pyautogui.rightClick(click_position['x'], click_position['y'])
            await callback.answer("👆 Правый!")
            await callback.message.edit_text("✅ Правый клик!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="autoclick_back")]]))
        else:
            await callback.answer("Сначала запомни позицию!")
    
    elif data == "autoclick_multi":
        await state.set_state(AutoClickerState.waiting_for_clicks)
        await callback.message.edit_text(
            "🖱️ МНОГОКРАТНЫЙ КЛИК\n\nВведи: количество, интервал(сек)\nНапример: 10, 1 (10 кликов через 1 сек)",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="autoclick_back")]])
        )
        await callback.answer()
    
    elif data == "autoclick_loop":
        await state.set_state(AutoClickerState.waiting_for_clicks)
        await callback.message.edit_text(
            "🖱️ БЕСКОНЕЧНЫЙ КЛИК\n\nВведи сколько кликов в секунду:\nНапример: 5\n\nОтправь 'стоп' для остановки.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="autoclick_back")]])
        )
        await callback.answer()
    
    elif data == "autoclick_stop":
        stop_autoclick()
        await callback.message.answer("⏹️ Остановлено", reply_markup=main_keyboard())
        await callback.answer()
    
    elif data == "autoclick_back":
        await state.clear()
        await cmd_autoclicker(callback.message, state)
        await callback.answer()


async def handle_autoclicker_input(message: types.Message, state: FSMContext):
    from app.keyboards import autoclicker_keyboard
    text = message.text.strip().lower()
    
    # Проверка стоп
    if text == "стоп" or text == "stop":
        stop_autoclick()
        await message.answer("⏹️ Остановлено", reply_markup=main_keyboard())
        await state.clear()
        return
    ...
    try:
        ...
        run_autoclick(cps)
        await message.answer(
            f"▶️ ВКЛ: {int(cps)} кликов/сек\n\nЖми кнопку СТОП для остановки.",
            reply_markup=autoclicker_keyboard()
        )
    except ValueError:
        await message.answer("Введи число или 'стоп'")
    except Exception as e:
        await message.answer(f"Ошибка: {e}")
