...
def main_keyboard(monitoring=False):
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="Система 🖥️"), KeyboardButton(text="Блокировка программ 🚫"), KeyboardButton(text="Автокликер 🖱️")],
            [KeyboardButton(text="Выключить 🔴"), KeyboardButton(text="Перезагрузить 🔄"), KeyboardButton(text="Сон 💤"), KeyboardButton(text="Заблокировать 🔒")],
            [KeyboardButton(text="Сменить пароль 🔑")],
            [KeyboardButton(text="Выкл бота 🛑"), KeyboardButton(text="Перезапустить 🔄"), KeyboardButton(text="Структура 📁")]
        ],
...
def programs_list_keyboard(programs, view_type):
    ...
    for i, prog in enumerate(steam_games[:8], 1):
        ...
        if len(btn_text) > 40:
            btn_text = btn_text[:37] + "..."
        keyboard.append([
            InlineKeyboardButton(text=f"{i}. {btn_text}", callback_data=f"run_{prog['id']}")
        ])
...
def delete_programs_keyboard(programs):
    """Клавиатура удаления программ"""
    keyboard = []
    for prog in programs:
        icon = "🎮" if 'steam' in prog.get('type', '') else "🖥️"
        btn_text = f"{icon} {prog['name']}"
...
def process_keyboard(pid, process_name):
    """Клавиатура для управления процессом"""
    return InlineKeyboardMarkup(
        inline_keyboard=[
...
def block_programs_keyboard(enabled=False):
    ...
    toggle_text = "⏹️ Выкл" if enabled else "▶️ Вкл"
    toggle_callback = "block_stop" if enabled else "block_start"
    
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="➕ Добавить", callback_data="block_add_exe")],
            [InlineKeyboardButton(text="📋 Список", callback_data="block_list")],
            [InlineKeyboardButton(text=toggle_text, callback_data=toggle_callback)],
            [InlineKeyboardButton(text="⬅️ Назад", callback_data="block_back")]
        ]
    )
...
def blocked_list_keyboard(programs):
    ...
    keyboard.append([InlineKeyboardButton(text="⬅️ Назад", callback_data="block_back")])
    return InlineKeyboardMarkup(inline_keyboard=keyboard)
...
def processes_keyboard(processes, page=0, per_page=10):
    """Клавиатура списка процессов с пагинацией"""
    keyboard = []
    ...
    for proc in page_processes:
        ...
        if len(proc_name) > 35:
            proc_name = proc_name[:32] + "..."
        
        keyboard.append([
            InlineKeyboardButton(text=f"{proc_name} ({proc_pid})", callback_data=f"process_{proc_pid}")
        ])
...
def autoclicker_keyboard():
    """Клавиатура автокликера"""
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="⏹️ СТОП", callback_data="autoclick_stop")]
        ]
    )
